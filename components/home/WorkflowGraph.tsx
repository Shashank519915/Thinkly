...
function GlassNode({ data }: { data: any }) {
  ...
  return (
    <div
      className={cn(
        "rounded-2xl bg-[#111111] border border-white/10 text-white relative transition-all duration-300 shadow-2xl overflow-hidden",
      ...
      style={{ width: NODE_W, borderLeft: `6px solid ${accent}` }}
    >
...
export function WorkflowGraph({
  ...
  const { layoutedNodes, initialEdges } = useMemo(() => {
    if (!rawNodes || rawNodes.length === 0) return { layoutedNodes: [], initialEdges: [] }
    ...
    const nodes: Node[] = rawNodes.map(n => ({
      id: n.id,
      type: 'custom',
      data: { ...n, runStatus: runLogs[n.id] } as any,
      position: { x: 0, y: 0 }
    }))
      ...
      nextArr.forEach((targetId: string) => {
        if (nodeIds.has(targetId)) {
          result.push({
            ...
            target: targetId,
            animated: true,
            style: { stroke: '#8b5cf6', strokeWidth: 3, filter: 'drop-shadow(0 0 8px rgba(139, 92, 246, 0.5))' },
      ...
      falseArr.forEach((targetId: string) => {
        if (nodeIds.has(targetId)) {
          result.push({
            ...
            animated: false,
            label: 'FALSE',
            labelStyle: { fill: '#f59e0b', fontWeight: 700, fontSize: 8 },
            labelBgStyle: { fill: 'rgba(0,0,0,0.6)' },
            style: { stroke: 'rgba(245,158,11,0.75)', strokeWidth: 2, strokeDasharray: '5 4' },
          })
        }
      })
      ...
      errorArr.forEach((targetId: string) => {
        if (nodeIds.has(targetId)) {
          result.push({
            ...
            animated: false,
            label: 'ERROR',
            labelStyle: { fill: '#f43f5e', fontWeight: 700, fontSize: 8 },
            labelBgStyle: { fill: 'rgba(0,0,0,0.6)' },
            style: { stroke: 'rgba(244,63,94,0.75)', strokeWidth: 2, strokeDasharray: '5 4' },
          })
        }
      })
      return result
    ...
    return { layoutedNodes: positioned, initialEdges: edges }
  }, [rawNodes, runLogs])
  ...
  const onInit = useCallback((instance: ReactFlowInstance) => {
    setTimeout(() => {
      ...
      if (trigger) {
        instance.setCenter(trigger.position.x + NODE_W / 2, trigger.position.y + NODE_H / 2, { zoom: 1, duration: 800 })
      }
    }, 150)
  }, [])
    ...
    <div className="w-full h-full min-h-[400px] relative">
      ...
      >
        ...
        <Controls
          ...
          style={{ width: 150 }}
          className="!flex !flex-row !justify-around !bg-black/95 !border-white/10 !shadow-[0_8px_32px_rgba(0,0,0,0.5)] !rounded-xl overflow-hidden scale-80 sm:scale-[0.95] !p-1 [&_button]:!bg-transparent [&_button]:!border-none [&_button_svg]:!fill-white/60 [&_button]:hover:!bg-white/10 [&_button]:!transition-colors !mb-[120px] sm:!mb-[135px] !ml-[20px] origin-bottom-left"
        />

        <MiniMap
          position="bottom-left"
          style={{ width: 150, height: 100 }}
          className="!bg-black/80 !border-white/10 !rounded-2xl !shadow-[0_8px_32px_rgba(0,0,0,0.4)] overflow-hidden scale-80 sm:scale-[0.95] origin-bottom-left !mb-[16px] sm:!mb-[20px] !ml-[20px]"
